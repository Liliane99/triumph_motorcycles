            ...
            try {
                ...
                const formattedUsers = usersData.map(user => ({
                    ...
                    name: user.role.value === 'admin' || user.role.value === 'manager'
                        ? `${user.username.value} (Triumph Motorcycle)`  // Pour admin et manager
                        : user.username.value  // Pour client, pas de modification
                }));
        ...
        <Form {...form}>
            <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
                ...
                <Button type="submit" className="w-full" disabled={loading}>
                    {loading ? "Enregistrement..." : mode === "create" ? "Ajouter une moto" : "Mettre à jour"}
                </Button>
            </form>
        </Form>
    );
