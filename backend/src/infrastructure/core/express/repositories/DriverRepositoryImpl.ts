...
export class DriverRepositoryImpl implements DriverRepository {
  ...
  private mapToDriver(prismaDriver: any): Driver {
    ...
    // Vérifie si `motorcycle` existe avant d'instancier `Motorcycle`
    ...
    return new Driver(
      ...
      prismaDriver.license_number,
      prismaDriver.experience_level,
      new Date(prismaDriver.date_of_birth),
      new User(
        prismaDriver.client.user_id, 
        prismaDriver.client.user_name,
        prismaDriver.client.email,
  ...
  async save(driver: Driver): Promise<Driver> {
    ...
    const dateOfBirthParsed = driver.dateOfBirth instanceof Date
      ? driver.dateOfBirth.toISOString()
      : new Date(driver.dateOfBirth).toISOString();
    ...
    console.log("➡️ Driver License Number:", driver.licenseNumber);
